...
export async function fetchChainData(): Promise<ChainData[]> {
  try {
    ...
    const cachedData = await getCachedData(CACHE_KEYS.CHAINS);
    if (cachedData) {
      console.log('Using cached chain data');
      return cachedData as ChainData[];
    }

    // If not in cache, fetch from API
    const [tvlResponse, mcapResponse] = await Promise.all([
      axios.get<ChainTVLData[]>(`${DEFILLAMA_API}/v2/chains`),
      axios.get<ChainMarketCapData[]>(`${DEFILLAMA_API}/v2/chains/market-cap`)
    ]);

    // Process and combine the data
    const chainData: ChainData[] = tvlResponse.data.map(chain => {
      const mcapData = mcapResponse.data.find(m => m.gecko_id === chain.gecko_id);
      return {
        id: chain.gecko_id,
        name: chain.name,
        ...
        marketCap: mcapData?.mcap || 0,
        mcapToTvl: mcapData?.mcap ? mcapData.mcap / chain.tvl : 0,
        protocols: chain.protocols || 0
      };
    });

    // Cache the data
    await setCachedData(CACHE_KEYS.CHAINS, chainData);
...
export async function fetchChainProtocols(chainName: string): Promise<ChainProtocol[]> {
  try {
    ...
    if (cachedData) {
      console.log('Using cached chain protocols data');
      return cachedData as ChainProtocol[];
    }

    // If not in cache, fetch from API
    const response = await axios.get<DefiLlamaProtocol[]>(`${DEFILLAMA_API_BASE}/protocols`);
    const protocols = response.data
      ...
      .map(p => ({
        ...
        tvl: p.tvl,
        change_24h: p.change_1d || 0,
        change_7d: p.change_7d || 0
      }))
      .sort((a, b) => b.tvl - a.tvl);

    // Cache the data
    await setCachedData(cacheKey, protocols);
