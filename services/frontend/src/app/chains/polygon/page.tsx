...
export default function PolygonPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('/api/chains/polygon');
        if (!response.ok) {
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <Card className="bg-background/50 border-border/50">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{formatValue(polygonData.tvl)}</div>
                      <p className={`text-sm ${polygonData.change_24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                        {polygonData.change_24h >= 0 ? '+' : ''}{polygonData.change_24h.toFixed(2)}% (24h)
                      </p>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-background/50 border-border/50">
                    ...
                    <CardContent>
                      <div className={`text-2xl font-bold ${polygonData.change_7d >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                        {polygonData.change_7d >= 0 ? '+' : ''}{polygonData.change_7d.toFixed(2)}%
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-background/50 border-border/50">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{polygonData.protocols?.length || 0}</div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-background/50 border-border/50">
                    ...
                    <CardContent className="flex items-center gap-2">
                      ...
                      />
                      <div className="text-2xl font-bold">{polygonData.tokenSymbol || 'MATIC'}</div>
                    </CardContent>
                  </Card>
                </div>
                
                <div>
                  <h2 className="text-2xl font-bold mb-4">Top Protocols on Polygon</h2>
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {polygonData.protocols && polygonData.protocols.length > 0 ? (
                      polygonData.protocols.slice(0, 9).map((protocol, index) => (
                        <motion.div
                ...
                <div className="flex justify-center mt-8">
                  <a 
                    href="https://defillama.com/chain/Polygon" 
                    target="_blank" 
